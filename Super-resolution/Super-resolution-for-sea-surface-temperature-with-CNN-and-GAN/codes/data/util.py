...
def read_img(env, path, size=None):
    ...
    if env is None:  # img
        img = np.load(path)  # cv2.imread(path, cv2.IMREAD_UNCHANGED)
    else:
    ...
    SST_MIN_TEMP = -5.0
    SST_MAX_TEMP = 40.0
    img = img.astype(np.float32) # 先转换类型
    img_clipped = np.clip(img, SST_MIN_TEMP, SST_MAX_TEMP)
    img = (img_clipped - SST_MIN_TEMP) / (SST_MAX_TEMP - SST_MIN_TEMP)
    ##修改归一化算法结束
    if img.ndim == 2:
