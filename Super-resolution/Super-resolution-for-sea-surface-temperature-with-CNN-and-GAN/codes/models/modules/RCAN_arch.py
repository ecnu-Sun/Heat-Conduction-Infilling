...
class RCAB(nn.Module):
    ...
    def __init__(self, num_feat, squeeze_factor=16, res_scale=1):
        ...
        # === 新增：定义FiLM层 ===
        # Nino3.4指数是1维的
        self.film = FiLMLayer(condition_dim=1, feature_channels=num_feat)
        # === 代码结束 ===
    ...
    #     return res + x
    # 修改 forward
    def forward(self, x, nino): # 接收nino作为额外参数
        res = self.conv1(x)
        res = self.relu(res)
        # print(nino)
        # === 新增：应用FiLM ===
        res = self.film(res, nino)
        # === 代码结束 ===
...
class RCAN(nn.Module):
    ...
    def forward(self, x,nino):
        ...
        # === 修改：手动循环以传递nino ===
        # 原有: res = self.conv_after_body(self.body(x))
        res = x
