...
class RB(nn.Module):
    def __init__(self, conv, n_feat, kernel_size, reduction, bias=True, bn=False, act=nn.ReLU(inplace=True), res_scale=1, dilation=2):
        ...
        #                                 )
        # === 修改：将 nn.Sequential 拆开，以便插入FiLM ===
        ...
        # === 新增：定义FiLM层 ===
        self.film = FiLMLayer(condition_dim=1, feature_channels=n_feat)
...
class LSRAG(nn.Module):
    ...
    #     return x
    #     ##
    # === 修改：forward 接收并传递 nino ===
    def forward(self, x, nino):
        residual = x
        
        # 手动循环，将 nino 传递给每一个 RB 块
        for i, l in enumerate(self.rcab):
            x = l(x, nino)
...
class SAN(nn.Module):
    ...
    #     return x 
    # === 修改：forward 接收 nino 并逐级传递 ===
    def forward(self, x, nino):
        x = self.head(x)
        
        # non_local模块不接收nino，因为它关注空间关系
        xx = self.non_local(x)

        residual = xx
        
        # 手动循环，将 nino 传递给每一个 LSRAG 组
        for i, l in enumerate(self.RG):
            xx = l(xx, nino) + self.gamma * residual
