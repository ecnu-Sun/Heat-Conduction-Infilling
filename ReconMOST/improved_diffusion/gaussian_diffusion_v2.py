...
class GaussianDiffusion:
    ...
    def p_mean_variance(
        self, model, x, t, clip_denoised=True, denoised_fn=None, dynamic_sample=None, model_kwargs=None
    ):
        """
        ...
        return {
            ...
            "guided_gradient": guided_gradient,
        }

    def _predict_xstart_from_eps(self, x_t, t, eps):
        # x0 = (x_t - sqrt(1-\bar\alpha) * eps) / sqrt(\bar\alpha)
    ...
    def condition_mean(self, cond_fn, p_mean_var, x, t, model_kwargs=None):
        ...
        if cond_fn is not None:
            gradient = cond_fn(x, self._scale_timesteps(t), p_mean_var, **model_kwargs)
    ...
    def condition_mean_on_next_x(self, model, x, t, clip_denoised=True, denoised_fn=None, model_kwargs=None):
        ...
        def process_xstart(x):
            ...
            if clip_denoised:
                return x.clamp(-1, 1)
            return x
        # # xt/mu(t+1)-> epsilon(t)->x0: x0 = (x_t - sqrt(1-\bar\alpha) * eps) / sqrt(\bar\alpha)
        pred_xstart = process_xstart(
            self._predict_xstart_from_eps(x_t=x, t=t, eps=model_output)
        )
        assert (
    ...
    def condition_score(self, cond_fn, p_mean_var, x, t, model_kwargs=None):
        ...
        eps = eps - (1 - alpha_bar).sqrt() * cond_fn(
            x, self._scale_timesteps(t), _extract_into_tensor(self.sqrt_recip_alphas_cumprod, t, x.shape) , **model_kwargs
        )
    ...
    def p_sample(
        ...
        if dynamic_guided:
            if dynamic_guided_with_next:
                ...
                out["mean"] = out["mean"].float() + out["variance"] * out["guided_gradient"].float()
        else:
            out["mean"] = self.condition_mean(
                cond_fn, out, x, t, model_kwargs=model_kwargs
            )
    ...
    def p_sample_loop(
        ...
        for sample in self.p_sample_loop_progressive(
            ...
            cond_fn=cond_fn,
            use_sigma=use_sigma,
            dynamic_guided=dynamic_guided,
            dynamic_guided_with_next=dynamic_guided_with_next,
            model_kwargs=model_kwargs,
            device=device,
            progress=progress,
        ):
    ...
    def p_sample_loop_progressive(
        ...
        for i in indices:
            ...
            with th.no_grad():
                ...
                #对以下代码不注释的时候就是使用land_set0了，需要和guided_sample_analysis_land_set0.py同时使用
                # if model_kwargs is not None and "land_mask" in model_kwargs:
                #     mask = model_kwargs["land_mask"]
                #     out["sample"][mask] = 0.0
                # 注意：对以下四行代码不注释的时候，就是启用硬引导了，需要和guided_sample_analysis_hard_guide同时使用
                # if model_kwargs is not None and "y" in model_kwargs:
                #     y = model_kwargs["y"]
                #     mask = ~th.isnan(y)  # 非NaN位置就是引导点
                #     out["sample"][mask] = y[mask]  # 直接替换为引导值
