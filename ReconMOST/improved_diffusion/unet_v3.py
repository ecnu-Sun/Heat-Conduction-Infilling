...
class UNetModel(nn.Module):
    ...
    def __init__(
        ...
        self.final_embed_dim = time_embed_dim * 2
        # 时间embed
    ...
    def forward(self, x, timesteps, y=None,**kwargs):
        ...
        #     # 将年和月的嵌入相加，形成一个完整的真实时间嵌入
        #     real_time_emb = self.year_embed(year_emb_sincos) + self.month_embed(month_sincos)
        # else:
        #     # 如果没有提供year/month，创建一个零张量以保持维度一致
        #     real_time_emb = th.zeros_like(time_emb)
        # ==================== 真实时间嵌 入新增代码结束 ====================
        # ==================== NINO指数嵌入新增代码开始 ====================
        nino = kwargs.get("nino")
        if nino is None:
            print("no nino,expect nino")
        # print(f"use nino:{nino}")
        # 将NINO指数乘以500后进行嵌入
        nino_scaled = nino * 500
        nino_emb_sincos = timestep_embedding(nino_scaled, self.model_channels)
        nino_emb =self.nino_embed(nino_emb_sincos)
        # emb = th.cat([time_emb, real_time_emb, nino_emb], dim=1)
        emb = th.cat([time_emb,nino_emb], dim=1)
        # print("usev3")
        # # ==================== NINO指数嵌入新增代码结束 ====================
        if self.num_classes is not None:
