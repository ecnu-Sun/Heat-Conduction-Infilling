...
def main():
    ...
    for _,row in df.iterrows():
        ...
        year=int(date[:4])
        month = int(date[5:7])
        nino_dict[(year, month)] = nino_value
    ...
    for key, guided_arr in guided_arr_dict.items():
        ...
        while len(all_images) * args.batch_size < args.num_samples:
            ...
            # 传递realtime开始
            date_str = key[-6:]
            print(f"Processing date string: {date_str}")
            year = int(date_str[:4])
            ...
            year = int(date_str[:4])
            month = int(date_str[4:])
            model_kwargs["nino"]=th.tensor([nino_dict.get((year, month), 0.0)]* args.batch_size, device=dist_util.dev())
            print(f"Using NINO value: {model_kwargs['nino']}")
            # #传递nino结束
            ...
            mse_3d = (sample - eval_y)**2  # (B, CHW)
            mse_3d = th.where(th.isnan(mse_3d), th.zeros_like(mse_3d), mse_3d)  # 将NaN替换为0
            if 'all_mse_3d' not in locals():
                all_mse_3d = []
            all_mse_3d.append(mse_3d.cpu().numpy())
            # 添加MSE空间计算 - 保存所有深度
        ...
        if dist.get_rank() == 0:  # 只让0号进程写日志，注意，这里的报告的pred_loss和guided_loss只是0号进程处理的samples的，不过单GPU下没问题
            ...
            # # 合并所有批次的MSE
            # all_mse_3d_concat = np.concatenate(all_mse_3d, axis=0)  # (total_samples, H, W, D)
            # avg_mse_3d = np.mean(all_mse_3d_concat, axis=0)  # (H, W, D)
            
            # # 选择要可视化的深度层（可以修改这些值）
            # depth_indices_to_plot = [0, 5, 10, 20, 30,41]  # 例如：表层、5米、10米、20米、30米
            # depths_meters = [5.02, 15.08, 25.16, 35.28, 45.45, 55.69, 66.04, 76.55, 87.27, 98.31,
            #                 109.81, 121.95, 135.03, 149.43, 165.73, 184.70, 207.43, 235.39, 270.53, 315.37,
            #                 372.97, 446.80, 540.50, 657.32, 799.55, 968.00, 1161.81, 1378.66, 1615.29, 1868.07,
            #                 2133.52, 2408.58, 2690.78, 2978.17, 3269.28, 3563.04, 3858.68, 4155.63, 4453.50, 4752.02,
            #                 5050.99, 5350.27]
            
            # # 创建多子图显示不同深度的MSE
            # n_depths = len(depth_indices_to_plot)
            # fig, axes = plt.subplots(n_depths, 1, figsize=(12, 4*n_depths))
            # if n_depths == 1:
            #     axes = [axes]
            
            # for idx, depth_idx in enumerate(depth_indices_to_plot):
            #     if depth_idx < avg_mse_3d.shape[2]:  # 确保深度索引有效
            #         mse_at_depth = avg_mse_3d[depth_idx, :, :]
            #         im = axes[idx].imshow(mse_at_depth, cmap='hot', origin='lower', aspect='auto',vmax=10)
            #         axes[idx].set_title(f'MSE at Depth Level {depth_idx} (~{depths_meters[depth_idx]:.1f}m) - {key}')
            #         axes[idx].set_xlabel('Longitude Index')
            #         axes[idx].set_ylabel('Latitude Index')
            #         plt.colorbar(im, ax=axes[idx], label='MSE (°C²)')
            
            # plt.tight_layout()
            # mse_plot_path = os.path.join(out_dir, f"mse_depth_maps_{key}.png")
            # plt.savefig(mse_plot_path, dpi=150, bbox_inches='tight')
            # plt.close()
            
            # # 也可以生成单个深度的图
            # single_depth = 0  # 选择要单独绘制的深度索引
            # fig, ax = plt.subplots(figsize=(12, 6))
            # mse_surface = avg_mse_3d[single_depth,:, : ]
            # im = ax.imshow(mse_surface, cmap='hot', origin='lower', aspect='auto')
            # ax.set_title(f'MSE at Surface (Depth {single_depth}, ~{depths_meters[single_depth]:.1f}m) - {key}')
            # ax.set_xlabel('Longitude Index')
            # ax.set_ylabel('Latitude Index')
            # plt.colorbar(im, label='MSE (°C²)')
            
            # surface_plot_path = os.path.join(out_dir, f"mse_surface_{key}.png")
            # plt.savefig(surface_plot_path, dpi=150, bbox_inches='tight')
            # plt.close()
            
            # # 保存3D MSE数组供后续分析
            # np.save(os.path.join(out_dir, f"mse_3d_{key}.npy"), avg_mse_3d)
            # logger.log(f"MSE depth maps saved to {mse_plot_path}")
            # logger.log(f"MSE surface map saved to {surface_plot_path}")
...
def create_argparser():
    defaults = dict(
        ...
        use_sigma=False,
        model_path="",
        sparse_data_path="/hdd/SunZL/data/ReconMOST-testdata",
        grad_scale=4.0,  # when 0: sample from the base diffusion model
        use_softmask=True,
